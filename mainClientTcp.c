...
int main(void)
{
	...
	SocketConfig_s socketConfigInitStruct =
	{
		...
		{
			.domainFamily = IP_V4,				   /*IP version 4 internet protocols family*/
			.ipAddr = ipV4Str("127.0.0.1"),		   /*Send/Recv/Connect from the loop back interface address*/
			.portNumber = (8081U)				   /*Set the port number to server's port number*/
		},
  ...
  while(1)
  {
		...
		errorStatus = sendData_tcp(socketConfigInitStruct.socketId,dataBuffer,(SocketBufferLen_t)(sizeof(dataBuffer)));
		...
		errorStatus = readData_tcp(socketConfigInitStruct.socketId,dataBuffer,(SocketBufferLen_t)(sizeof(dataBuffer)));
